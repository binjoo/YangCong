...
class YangCong_Widget_WeChat extends Widget_Abstract implements Widget_Interface_Do {
    ...
    public function isEvent(){
        if($this->postObj->Event == "subscribe"){
            $this->result->setText(isset($this->options->YangCong_welcome) ? $this->options->YangCong_welcome : '')->setMsgType(MessageTemplate::TEXT)->send();
        }
    }
    /**
     * 文本消息
    ...
    public function isText(){
        ...
        }else if(isset($this->options->YangCong_thirdPartySearch) && $this->options->YangCong_thirdPartyUrl && $this->options->YangCong_thirdPartyToken && $this->options->YangCong_thirdPartySearch) { //第三方处理
            $this->thirdParty();
        }else{
            $this->result->setText(isset($this->options->YangCong_notfound) ? $this->options->YangCong_notfound : '完全不明白你在说什么！')->setMsgType(MessageTemplate::TEXT)->send();
        }
    }
    /**
     * 图片消息
    ...
    public function thirdParty(){
        ...
        $params['signature'] = $this->options->YangCong_thirdPartyToken;
        ...
        ->setQuery($params)
        ->setData($postStr)
        ->send($this->options->YangCong_thirdPartyUrl);
    ...
    public function addonsAction($action, $postObj, $params = NULL){
        $file = __TYPECHO_ROOT_DIR__ . '/' . __TYPECHO_PLUGIN_DIR__ . '/' . 'YangCong/Addons' . '/'.$action;
        include_once $file;
