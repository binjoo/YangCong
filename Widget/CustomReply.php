...
class YangCong_Widget_CustomReply extends Widget_Abstract implements Widget_Interface_Do {
    ...
    public function form($action = NULL) {
        if (isset($this->request->rid) && 'insert' != $action) {
            ...
            if (!$custom) {
                $this->response->redirect(Helper::url('YangCong/Page/CustomReply.php', $this->options->adminUrl));
            }
        }
        ...
        if(count($addonsArray)){
            ...
            foreach ($addonsArray as $key => $row) {
                $selected = '';
                if (isset($custom['command']) && $custom['command'] === $key) {
                    $selected = ' selected="true"';
                }
                $addonsSelect .= '<option value="' . $key . '"' . $selected . ' data-param="' . (isset($row['param']) && $row['param'] == 'true' ? 1 : 0) . '">' . $row['name'] . '</option>';
            }
            $addonsSelect .= '</select>';
    ...
    public function updateCustomReply(){
        ...
        /** 转向原页 */
        $this->response->redirect(Helper::url('YangCong/Page/CustomReply.php&page='.$this->_currentPage, $this->options->adminUrl));
    }
